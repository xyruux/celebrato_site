<template>
    <div>  
        <span>Slides</span><br><hr><br>
        <v-card flat style="min-height: 20rem; max-height: 40rem;"> 
            <v-card flat class=" boarder pa-2"   style="overflow-y: auto;" :style="$vuetify.display.mobile?'max-height: 10rem; min-height: 10rem; ': 'max-height: 15rem; min-height: 20rem;'">
            <v-row dense v-if="images.length > 0">  
                ...
                <template v-if="$store.state.form_theme.demo?.collage[1]">
                    <v-col cols="4" md="3" v-if="!images.includes($store.state.form_theme.demo?.collage[1])"  >
                        <v-btn   @click="$store.state.form_theme.demo.collage =[]"   :height="$vuetify.display.mobile?'95':'125'" width="100%" variant="outlined" > <p>Demo</p>
                        ...
                        <v-tooltip location="top" :text="decodeURIComponent((v as string)?.split('%7E')[1] +'.'+ v.split('.')[4])">
                            <template v-slot:activator="{ props }">
                                <v-img v-bind="props" @click="selectImage(v)" height="100" class="text-center pa-4"  style="object-position: top;" cover :src="v"> 
                                    <v-icon color="primary" style="text-shadow:  -1px -1px 0 black, 1px -1px 0 black,  -1px  5px 0 black, 1px  5px 0 black;" size="50" v-if="$store.state.form_theme.demo?.collage.includes(v )">mdi-check-circle</v-icon>
                                </v-img> 
                            </template>
                        </v-tooltip>
...
export default defineComponent({ 
    ...
    methods: {
        ...
        selectImage(v: string){
            if(!this.$store.state.form_theme.demo?.collage.includes(v)){
                this.$store.state.form_theme.demo?.collage.push(v)
            }else{
                this.$store.state.form_theme.demo?.collage.splice(this.$store.state.form_theme.demo?.collage?.indexOf(v),1)
            }    
        },
        async current_id(){
            try {
