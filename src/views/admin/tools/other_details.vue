<template>
    <div>
        ...
        <v-card-text style="min-height: 20rem; max-height: 40rem; overflow-y: auto;" >
            <v-btn @click="type = 1" :color="type == 1?'primary':''" class="mx-1" variant="outlined">Card</v-btn>  
                    ...
                    <v-col cols="12" md="12">
                        <v-row>
                            <v-col cols="12" class="text-right">
                                <v-btn class="my-0 py-0 mx-1"   variant="outlined" @click="form['portion'].push([])">
                                <v-icon start>mdi-plus</v-icon>  Add
                                </v-btn>
                            </v-col>
                            <v-col cols="12">
                                <v-card color="transparent" class="pa-4 my-2 border border-primary rounded-xl" v-for="(v,k) in form['portion']" :key="k">
                                    <v-row>
                                        <v-col class="text-right">
                                        <v-btn variant="text" @click="form['portion'].splice(form['portion'].indexOf(v),1)"><v-icon>mdi-close</v-icon></v-btn>
                                        </v-col>
                                            <v-col cols="12" >
                                                <span>Title: </span>
