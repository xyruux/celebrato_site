...
export default defineComponent({
  ...
  mounted() {
    ...
    this.dropzoneInstance.on("sending", (file, xhr, formData) => {
      // console.log("Adding additional parameters:", this.additionalParams);
    ...
    onSuccess(file: Dropzone.DropzoneFile, response: any) {
      ...
      if(response.type == 'image'){  
        ...
        response.data.forEach(element => { 
          console.log(element)
          this.$store.state.form.file_names.images.push(element) 
        });
          
        this.$store.state.form.file_names.images = [
          ...new Set(this.$store.state.form.file_names.images.map((img: any) => JSON.stringify(img))),
        ].map((img: string) => JSON.parse(img));
 
      }
      if(response.type == 'video'){
        this.$store.state.form.file_names.video = response.data
      }
    },
